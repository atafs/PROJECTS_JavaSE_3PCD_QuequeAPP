package com.iscte.queque.test.thread._1;
...
public class ServerService implements Runnable {
	...
	//COMPONENT
	public static DefaultListModel<Object> list_clients_model;
	...
	public void connect_sendMessage() {
		while(true) {
			try {
				...
				username = (String) this.reader.readObject();
	...
	/** */
	public void data() {
		list_clients_model.addElement(username + " - " + client.getInetAddress().getHostAddress() + " - " + client.getInetAddress().getHostName());
		list_cliente_states.add(0);
		
		list_data.add(new DataPackage());
		list_sockets.add(client);
	}
}
